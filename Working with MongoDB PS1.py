...
def process_file(filename, fields):
    classification_fields = ['family_label', 'class_label', 'phylum_label', 'order_label', 'kingdom_label', 'genus_label']
        ...
        for line in reader:
            ...
            for field in process_fields:
                ...
                if field == 'rdf-schema#label':
                    start = line[field].find( '(' )
                    end = line[field].find( ')' )
                    if start != -1 and end != -1:
                        line[field] = line[field][:start] + line[field][end+1:]
                    datum[fields[field]] = line[field].strip()
                elif line[field] == 'NULL' and field != 'name':
                    if field in classification_fields:
                        classifications[fields[field]] = None
                    else:
                        datum[fields[field]] = None
                elif field == 'name' and (line[field] == 'NULL' or not line[field].isalnum()): 
                    start = line['rdf-schema#label'].find( '(' )
                    end = line['rdf-schema#label'].find( ')' )
                    if start != -1 and end != -1:
                        line['rdf-schema#label'] = line['rdf-schema#label'][:start] + line['rdf-schema#label'][end+1:]
                    datum[fields[field]] = line['rdf-schema#label'].strip()
                elif field == 'synonym':
                    datum[fields[field]] = parse_array(line[field])
                else:
                    if field in classification_fields:
                        classifications[fields[field]] = line[field]
                    else:
                        datum[fields[field]] = line[field]
            datum['classification'] = classifications
            data.append(datum)
    return data
